...
export default function Memories() {
  ...
  const handleDelete = id => {
    try {
      fetch(`http://193.203.162.218:4200/api/v1/deletememories/${id}`, {
        method: "DELETE"
      })
  ...
  const updateUser = async id => {
    ...
    const image = `http://193.203.162.218:4200/uploads/memories/${user.memoriesImg}`
    setShowImg(image)
    setData(user);
    setEdit(true);
  }
  ...
  const handleUpdate = async () => {
    try {
      const addRecordEndpoint = `http://193.203.162.218:4200/api/v1/editmemories/${data._id}`;
      const formDataImg = new FormData();

      for (let i = 0; i < data.memoriesImg.length; i++) {
        formDataImg.append('memoriesImg', data.memoriesImg[i]);
        formDataImg.append('memoriesName', data.memoriesImg[i].name);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const addRecordEndpoint = 'http://193.203.162.218:4200/api/v1/addmemories';
      const formDataImg = new FormData();
      for (let i = 0; i < data.memoriesImg.length; i++) {
        formDataImg.append('memoriesImg', data.memoriesImg[i]);
        formDataImg.append('memoriesName', data.memoriesImg[i].name)
        ...
        <div class="w-full max-w-xs mx-auto py-4 bg-gray-200 my-4 px-4 shadow-xl rounded-xl">
          <div class=" py-2">
            <input class="w-full border border-gray-400 p-2 focus:outline-none rounded-md text-black"
              type="file" name='memoriesImg' onChange={handleChange} />
                        ...
                        <tr className='border border-grey-400'>
                          ...
                          <td class="px-6 py-4 whitespace-no-wrap  bg-white-200 border-r border-gray-400">
                            <div class="text-sm leading-5 text-gray-900">
                              <img src={`http://193.203.162.218:4200/uploads/memories/${item.memoriesImg}`} className="block h-auto w-full" />
                            </div>
                          </td>
